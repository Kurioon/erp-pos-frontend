...
import PendingPayments from '@/components/finance/PendingPayments.vue'

const tabs = [
  { id: 'journal', label: 'Журнал транзакцій', component: shallowRef(TransactionJournal) },
  { id: 'pending', label: 'Очікують дооплати', component: shallowRef(PendingPayments) },
...
<style scoped>
.finance-view { padding: 32px; background-color: #ffffff; min-height: 100vh; display: flex; flex-direction: column; }
.page-header { margin-bottom: 24px; }
.header-info h1 { font-size: 1.6rem; color: #0f172a; margin: 0 0 4px 0; font-weight: 700; }
.subtitle { margin: 0; color: #64748b; font-size: 0.9rem; }
.tabs-navigation { display: flex; gap: 28px; border-bottom: 1px solid #e2e8f0; margin-bottom: 28px; }
.tab-link { background: transparent; border: none; padding: 0 0 12px 0; font-size: 0.95rem; font-weight: 600; color: #64748b; cursor: pointer; position: relative; transition: color 0.15s ease; }
.tab-link:hover { color: #0f172a; }
.tab-link.is-active { color: #2563eb; }
.tab-link.is-active::after { content: ''; position: absolute; bottom: -1px; left: 0; width: 100%; height: 2px; background-color: #2563eb; border-radius: 2px; }
.tab-content-wrapper { flex: 1; }
