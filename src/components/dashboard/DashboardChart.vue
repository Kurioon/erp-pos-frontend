<template>
  <div class="dashboard-block-card chart-container-block">
    ...
    <div class="pure-css-chart" aria-hidden="true">
      <div class="chart-y-axis">
        <span>260k</span><span>195k</span><span>130k</span><span>65k</span><span>0</span>
      </div>
      <div class="chart-bars-wrapper">
        <div v-for="(height, idx) in props.chartData" :key="idx" class="chart-bar-column">
          ...
          <span class="bar-day-label">{{ WEEK_DAYS_SHORT[idx] }}</span>
        </div>
      </div>
    </div>
  </div>
...
import { WEEK_DAYS_SHORT } from '@/constants/date'
...
.chart-y-axis { display: flex; flex-direction: column; justify-content: space-between; height: 190px; font-size: 0.75rem; color: #94a3b8; font-weight: 600; text-align: right; width: 35px; flex-shrink: 0; }
...
@media (max-width: 640px) {
  .bar-actual { width: 20px; }
  .chart-y-axis { display: none; }
  .pure-css-chart { gap: 8px; }
}
</style>
